                       ...
                       const bool& extension)
{
    ...
    const int64_t expiresAt = std::chrono::duration_cast<std::chrono::seconds>(
        (time + std::chrono::hours{(extension ? 30 : 1) * 24}).time_since_epoch()).count();
...
bool JWT::verifyToken(const ::jwt::decoded_jwt<::jwt::traits::kazuho_picojson>& jwt)
{
    ...
    auto jwtVerifier = ::jwt::verify()
                       ...
                       .allow_algorithm(::jwt::algorithm::hs256{
                           app().getCustomConfig()["jwt"]["private_key"].asString()
                       });
...
void JWT::addClaimToAttributes(std::map<std::string, std::any>& attributes,
